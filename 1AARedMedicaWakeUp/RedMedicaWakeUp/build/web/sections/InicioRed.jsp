...
<%
    ...
    request.setAttribute("name", "holaviejo");
...
<html lang="en">
    ...
    <body style="background-image: url('<%=request.getContextPath()%>/imagenes/gradient_bg.jpg'); background-repeat: no-repeat; background-attachment: fixed; background-size: cover;">
        ...
        <%
            ...
            String estado = "";
            String docpremiumhtml = "";
            while (datospersona.next()) {
                estado = datospersona.getString("idestado");
                if (datospersona.getInt("idtipo") == 2) {
                    docpremiumhtml = "&nbsp;<span class='icomoon icon-folder-plus small'></span>";
                }
            }
        ...
        <%
            ...
            if (opc == null) {
                ...
                if (buscado.equals("")) {
                    response.sendRedirect(response.encodeRedirectURL(request.getContextPath() + "/sections/InicioRed.jsp"));
                } else {
                    response.sendRedirect(response.encodeRedirectURL(request.getContextPath() + "/sections/InicioRed.jsp?Usr=" + buscado + ""));
                }
        ...
        <%
            ...
            if (publicacion != null) {
                catego = con.consulta("call newpost('" + UsrId + "','0','" + cifrado.stremplaza(publicacion) + "','2','1')");
                publicacion = null;
                if (catego.next()) {
            ...
            if (comentario != null) {
                catego = con.consulta("call newpost('" + UsrId + "','" + idpublicacion + "','" + cifrado.stremplaza(comentario) + "','2')");
                comentario = null;
                if (catego.next()) {
                    if (buscado.equals("")) {
                        response.sendRedirect(response.encodeRedirectURL(request.getContextPath() + "/sections/InicioRed.jsp"));
        ...
        <div id="wrapper">
            ...
            <div id="page-content-wrapper">
                <%
                    ...
                    ResultSet rsinfousu = rsinfousu = con.consulta("call validaRelaciones('1','" + UsrId + "','" + buscado2 + "')");
                    String imagenusu = "";
                    String nombreusu = "";
                    String apeusu = "";
                    String correousu = "";
                ...
                <form action="chat.jsp" method="post" class="usuario">
                    ...
                    <%
                        if (rsinfousu.next()) {
                            if (rsinfousu.getInt("esconfianza") == 1) {
                    %>
                    <div class="verInfo puntero" onclick="verinfo('<%=correousu%>', '<%=telusu%>')"><a style="outline: none;" href="#">Ver Info...</a></div>
                    <%-- <div class="infoUsu">Email: <%=correousu%></div>
                    <div class="infoUsu">Tel: <%=telusu%></div> --%>
                    <%
                            }
                ...
                <%
                    ...
                    try {
                        ...
                        if (buscado.equals("") && specificPost.equals("")) {
                            adonde = "0";
                            rs = con.consulta("call traepost('0','0','0')");
                %>
                <div  class="yoposteo">
                    <form name="posteaform" method="post" >
                        <textarea id="postealotext" class="areatext tcomment" placeholder="Como se siente?" name="publicacion" rows="2"></textarea>
                    </form>

                </div>
                <%
                    } else if (!buscado.equals("")) {
                        rs = con.consulta("call traepost('1','" + buscado + "','0')");
                        adonde = buscado;
                    } else if (!specificPost.equals("")) {
                        rs = con.consulta("call traepost('7','" + specificPost + "','0')");
                        adonde = "0";
                    }
                    String unico = "0";
                    String nounico = "z";
                                ...
                                $('form').each(function () {
                                    $(this).find('.tcomment2').keypress(function (e) {
                                        ...
                                        if (e.which === 10 || e.which === 13) {
                                            ...
                                            //////////////////////////////////////////////////
                                            $.post("../solicitudes", {opc: 18, idpublicacion: idpub, comentario: comentario}).done(function (data52) {
                                                ...
                                                if (data52.estatus === "ok") {
                                                    ...
                                                    $("#" + idareacomentario).before("<div id=\"comentariocreado" + idunicocomentariosnuevos + "\" onmouseover=\"desplega(document.getElementById('" + idunicocomentariosnuevos + "'))\" onmouseout=\"nodesplega(document.getElementById('" + idunicocomentariosnuevos + "'))\"data-toggle=\"tooltip\" data-placement=\"left\" title=\"En este momento\" style=\"position:relative;\">"
                                                            ...
                                                            + "<div class=\"anamecomment\"><a href=\"?Usr=<%=UsrId%>\"><%=nombreusu + " " + apeusu%><%=docpremiumhtml%>:</a>"
                                                            + "&nbsp;" + data52.msj + "</div>"
